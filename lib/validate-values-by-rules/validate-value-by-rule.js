...
function validateValueByRule(accumulator, value, index, collection, key, property, rule){
  ...
  switch(rule.type) {
    case 'equal':
      if(!_.isString(propertyValue) && !_.isNumber(propertyValue) && !_.isBoolean(propertyValue))
    ...
    case 'list':
      ...
      _.each(propertyValue, function(listValue){
        if(!_.isString(listValue) && !_.isNumber(listValue) && !_.isBoolean(listValue))
      ...
      break;
    case 'percent':
      if(!_.isNumber(propertyValue) || propertyValue > 100 || propertyValue < 0)
